...
percent_change <- function(stock_ticker, start_date, end_date){
  ...
  if(stringrcheck(str_detect(stock_ticker, "[[:upper:]]") == FALSE)){
...
profit_viz <- function(stock_ticker, start_date, end_date, benchmark_ticker){
  ...
  if (typeof(stock_ticker) != character) {
    stop("stock_ticker should be a string!")
  }

  if (typeof(benchmark_ticker) != character) {
  ...
  profit <- refresh2::melt(profit, id.vars="Date")
  ...
  profit_plot <- ggplot2::ggplot(data=profit, aes(x=Date, y= value, colour = variable)) +
    geom_line() +
